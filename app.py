...
# 1. Criar a pasta de cache se não existir (Essencial no Streamlit Cloud)
...
def gerar_video_longo(duracao_segundos=90, fps=30):
    ...
    # 2. Carregar sessão (GP do Brasil 2024, Corrida)
    session = fastf1.get_session(2024, 'Interlagos', 'R')
    session.load(telemetry=True)

    # 3. Pegar telemetria das voltas 50 a 53 de dois pilotos
    # Usando .pick_laps(range(50, 54)) correto do FastF1
    laps_p1 = session.laps.pick_driver('VER').pick_laps(range(50, 54))
    laps_p2 = session.laps.pick_driver('NOR').pick_laps(range(50, 54))

    tel1 = laps_p1.get_telemetry()
    tel2 = laps_p2.get_telemetry()

    # 4. Reamostrar as coordenadas X e Y para o total de frames desejado
    ...
    # 5. Criar a Animação 9:16 (TikTok)
